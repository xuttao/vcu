 ...
 * @Description: ...
 * @LastEditTime: 2021-05-09 18:24:56
 */
...
public:
        ...
        BoxInfo2 *getDetectBoxInfo(char *pData, int src_w, int src_h, int &num);
...
private:
        ...
        inline void getYoloBox2(box &b, float *x, int n, int index, int col, int row, int lw, int lh)
        {
                b.x = (col + x[index]) / lw;
                b.y = (row + x[index + 1]) / lh;
                float wtemp = x[index + 2];
