 ...
 * @Description: ...
 * @LastEditTime: 2021-03-19 17:02:24
 */
...
BoxInfo2 *NetDetect::getDetectBoxInfo(char *pData, int src_w, int src_h, int &num)
{
        // float thresh = thresh;
        float hier = iou;
        inputData = pData;
...
detection *NetDetect::makeNetworkBoxes(float thresh, int &num)
{
        ...
        if (detectArr)
        {
                for (int i = 0; i < last_box_num; i++)
                {
                        if (detectArr[i].prob)
                        {
                                delete[] detectArr[i].prob;
                                detectArr[i].prob = nullptr;
                        }
                }
                delete[] detectArr;
                detectArr = nullptr;
                ...
                for (int j = 0; j < signle_chanel_size; j++)
                {
                        ...
                        for (int n = 0; n < layer.grid_box_num; n++)
                        {
                                ...
                                if (objectness > thresh && count < maxBoxNum)
                                {
                                        ...
                                        getYoloBox2(dets[count].bbox, pData, layer.mask[n], box_index, col,
                                                    row, layer.net_w, layer.net_h);
#else
